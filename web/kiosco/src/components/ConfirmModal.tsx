...
export default function ConfirmModal({ orderId, roomNumber, checkIn, checkOut, subtotal, onClose }: Props) {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-navy/40 backdrop-blur-sm p-4" onClick={onClose}>
      <div
        className="animate-pop bg-white rounded-2xl p-6 max-w-sm w-full text-center shadow-[0_24px_60px_-20px_rgba(15,23,42,0.4)]"
      ...
      >
        ...
        <button
          onClick={onClose}
          className="tap-scale mt-4 w-full bg-navy text-white rounded-lg py-3 font-extrabold text-[length:var(--fs-body)] uppercase tracking-wide hover:bg-navy/90 transition-colors shadow-[0_4px_20px_rgba(15,23,42,0.25)]"
        >
          Cerrar
        </button>
      </div>
