...
export default function SuccessModal({ visible, order, onClose }: Props) {
  ...
  useEffect(() => {
    if (visible) { zoom.setValue(0); Animated.timing(zoom, { toValue: 1, duration: 300, useNativeDriver: true }).start(); }
  }, [visible, zoom]);
...
const s = StyleSheet.create({
  ...
  card: { backgroundColor: colors.brandPrimary, borderRadius: radii.card, padding: spacing.xl, width: '100%', maxWidth: 440, alignItems: 'center', borderWidth: 1, borderColor: colors.border },
  ...
  title: { fontFamily: typography.serifBold, fontSize: sizes.title, color: colors.brandCream, marginBottom: 4 },
  subtitle: { fontSize: sizes.cardSubtitle, color: colors.textMuted, marginBottom: spacing.lg },
  ...
  rowLabel: { fontSize: sizes.cardSubtitle, fontWeight: '600', color: colors.brandCream },
  rowValue: { fontSize: sizes.cardSubtitle, color: colors.textMuted },
  closeBtn: { backgroundColor: colors.brandAccent, borderRadius: radii.button, paddingVertical: 14, paddingHorizontal: 48 },
  closeTxt: { color: colors.brandPrimary, fontSize: sizes.cta, fontWeight: '600', fontFamily: typography.sansMedium },
});
